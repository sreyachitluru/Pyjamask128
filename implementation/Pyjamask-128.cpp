...
void printHex(const string &s) {
    for (unsigned char c : s) {
        cout << "\\x" << setw(2) << setfill('0') << hex << (int)c;
    }
    cout << endl;
}

// Function to convert unsigned char array to string
string convertToString(unsigned char arr[], int size) {
    stringstream ss;
...
int main() {
    unsigned char ip_key[] = { 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f };
    unsigned char ip_nonce[] = { 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B };
    unsigned char ip_associatedData[] = { 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E, 0x0F };
    unsigned char ip_plaintext[] = { 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E, 0x0F };

    string key = convertToString(ip_key, sizeof(ip_key));
    string nonce = convertToString(ip_nonce, sizeof(ip_nonce));
    string associatedData = convertToString(ip_associatedData, sizeof(ip_associatedData));
    string plaintext = convertToString(ip_plaintext, sizeof(ip_plaintext));
    ...
    cout << "\nDecrypted Message: "<<decryptionOutput<<"\n";
